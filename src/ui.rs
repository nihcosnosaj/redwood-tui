...
fn render_dashboard_view(f: &mut Frame, app: &App) {
    ...
    let items: Vec<ListItem> = app
            ...
            .map(|(i, fl)| {
                ...
                ListItem::new(Line::from(vec![
                    ...
                    Span::styled(format!(" │ {}", short_op), Style::default().fg(Color::DarkGray)),
                ]))
            })
            .collect();
    ...
    if let Some(fl) = app.flights.get(app.selected_index) {
        ...
        let seconds_ago = app.last_update
            .map(|inst| now.duration_since(inst).as_secs())
            .unwrap_or(0);

        // Color code the "Freshness"
        let latency_color = if seconds_ago < 40 { Color::Green } else { Color::Red };
        ...
        let stats_content = vec![
            Line::from(vec![
                ...
                Span::styled(format!("{}s", seconds_ago), Style::default().fg(latency_color)),
                Span::raw("  │  "),
                Span::styled("DB HITS: ", Style::default().add_modifier(Modifier::BOLD)),
                Span::styled(format!("{}/{}", app.db_match_count, app.flights.len()), Style::default().fg(Color::Cyan)),
            ]),
            Line::from(""), // Spacer
            Line::from(vec![
                ...
                Span::raw(if fl.registration.is_some() { "ENRICHED" } else { "RAW DATA" }),
            ]),
        ];
    ...
    if let Some(fl) = app.flights.get(app.selected_index) {
        ...
        let details = vec![
            Line::from(vec![
                Span::styled("Registration: ", Style::default().add_modifier(Modifier::BOLD)),
                Span::styled(fl.registration.as_deref().unwrap_or("N/A"), Style::default().fg(Color::Yellow)),
                Span::raw("  |  "),
            ...
            Line::from(vec![
                ...
                Span::raw(format!(" ({})", fl.aircraft_type.as_deref().unwrap_or("---"))),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled("Telemetry:    ", Style::default().add_modifier(Modifier::BOLD)),
                Span::raw(format!("{:.0} m  |  {:.0} km/h  |  {:.0}°", fl.altitude, fl.velocity, fl.true_track)),
            ]),
            Line::from(vec![
                Span::styled("Origin:       ", Style::default().add_modifier(Modifier::BOLD)),
...
fn render_loading_screen(f: &mut Frame, app: &App) {
    ...
    let gauge = Gauge::default()
        .block(Block::default().title(" Initializing Aircraft Database ").borders(Borders::ALL))
...
fn render_settings_view(f: &mut Frame, _app: &App) {
    ...
    f.render_widget(Paragraph::new(text).block(block).alignment(Alignment::Center), f.size());
}

/// Brand Identity via Colors
fn get_operator_color(operator: &str) -> Color {
    let op = operator.to_lowercase();
    if op.contains("united") { Color::Blue }
    else if op.contains("southwest") { Color::Yellow }
    else if op.contains("delta") { Color::Rgb(180, 20, 40) } // Dark Red
    else if op.contains("american") { Color::Cyan }
    else if op.contains("alaska") { Color::Rgb(0, 66, 110) } // Deep Blue
    else if op.contains("fedex") { Color::Magenta }
    else if op.contains("ups") { Color::Rgb(80, 40, 0) } // Brown
    else { Color::White }
}