...
impl FlightProvider {
    pub fn new() -> Self {
        Self { client: Client::builder().timeout(std::time::Duration::from_secs(10)).build().unwrap() }
    }
    ...
    pub async fn fetch_overhead(&self, lat: f64, lon: f64) -> Result<Vec<Flight>> {
        ...
        let res = self.client.get(url).send().await?.json::<OpenSkyResponse>().await?;
        
        let flights = res.states
            .unwrap_or_default()
            .into_iter()
            .map(Flight::from)
            .collect();
