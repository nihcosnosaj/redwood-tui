...
pub fn render(f: &mut Frame, app: &App) {
    ...
    // --- Sidebar ---
    let items: Vec<ListItem> = app.flights.iter().enumerate().map(|(i, fl)| {
    ...
    let list = List::new(items)
        .block(Block::default().title(" Flights Nearby ").borders(Borders::ALL).border_type(BorderType::Rounded));
    ...
    let anim = Paragraph::new(cloud_art)
        ...
        .block(Block::default().title(" Radar Status ").borders(Borders::ALL));
    f.render_widget(anim, main_chunks[0]);

    // 2. Flight Details
    if let Some(fl) = app.flights.get(app.selected_index) {
        let details = vec![
            Line::from(vec![Span::raw("Callsign: "), Span::styled(&fl.callsign, Style::default().fg(Color::Yellow))]),
        ...
        let p = Paragraph::new(details)
            .block(Block::default().title(" Flight Info ").borders(Borders::ALL).padding(Padding::new(2, 2, 1, 1)));
